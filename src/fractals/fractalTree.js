...
export function createFractalTree(depth) {
    ...
    function buildBranch(parent, length, currentDepth, maxDepth) {
        if (length < 0.2 || currentDepth > maxDepth) return;
        ...
        for (let i = 0; i < numBranches; i++) {
            ...
            child.rotation.x = THREE.MathUtils.randFloat(-Math.PI / 6, Math.PI / 6);
            child.rotation.z = THREE.MathUtils.randFloat(-Math.PI / 6, Math.PI / 6);
            child.position.y = length;

            container.add(child);
            const scale = THREE.MathUtils.randFloat(0.6, 0.9);
            buildBranch(child, length * scale, currentDepth + 1, maxDepth);
        }
    }
